...
def latest_audit(project_id: str, request: Request,
    ...
    empty store → empty list (no audit has run yet)."""
...
def rerun_audit(project_id: str, payload: dict, request: Request):
    ...
    results, _spent = run_audit(root, project_id,
                                f"rerun-{datetime.now(timezone.utc).isoformat()}",
                                claim_filter=claim_id)
    n = len(store.list_audit_runs()) + 1
    run = AuditRun(id=f"A-{n:03d}",
