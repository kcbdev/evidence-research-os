...
- Carry-forwards (for future-PBI executors, not fixed here): PBI-065 to decide `@hookform/resolvers` (needed for canonical RHF+zod+Field Controller pattern, else hand-roll safeParse); PBI-068 to decide `@codemirror/lang-javascript`; PBI-069 card says Budget tab is a "plain Form" — read as HTML form + Field, not the removed wrapper; PBI-070 must mount `<Toaster/>` (layout untouched by design — wiring it now would have violated no-route-edits) or ValidationToastList is inert; TooltipProvider needs no global wrap (per-instance works).
